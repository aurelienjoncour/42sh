...
int subst_undefined_argv(char *label, token_t *tok, size_t idx)
{
    if (my_str_isnum(label) == true) {
        if (process_subst_value_force(label, tok, "", idx) == EXIT_ERROR) {
            return EXIT_ERROR;
        }
        return EXIT_SUCCESS;
    }
